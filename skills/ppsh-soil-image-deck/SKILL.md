---
name: ppsh-soil-image-deck
description: >
  SOIL 純圖片教學簡報技能。遵循 SOIL Teaching Deck Workflow 六顆引擎的教學設計邏輯，
  但每一頁都是由 AI 生圖模型完整生成的 PNG 圖片（包含標題、內文、視覺），
  最後打包成 .pptx（每張 slide 是一張全版圖）。
  當使用者說「做純圖片簡報」、「全圖簡報」、「每頁都是 AI 生的圖」、
  「用 AI 生一整份簡報」、「做一份像海報一樣的教學簡報」、
  「快速做一份視覺震撼的簡報」或任何要求「整頁都是 AI 圖像」的教學簡報需求時，
  請一定要使用此技能。
  本技能與 ppsh-soil-teaching-deck 不同：ppsh-soil-teaching-deck 產出「文字可編輯 + AI 插圖」的
...
> - `ppsh-soil-image-deck`（本技能）：引擎 6 逐頁生出「整頁圖」，打包成 pptx（每 slide 就是一張 full-bleed 圖）
...
**不適合**：需要台下老師後續修改文字、有大量精細資料、需要動畫互動——這些用 `ppsh-soil-teaching-deck` 或 `pptx`。
...
同 ppsh-soil-teaching-deck 引擎一。產出：總概念、三個子概念、三個常見誤解、帶走一句話、最小事實包。
...
同 ppsh-soil-teaching-deck 引擎二。三段節奏（引起動機／維持注意／喚起行動），比例 2:6:2 左右（10 頁時）。
...
- 頁碼
- 頁面角色（同 ppsh-soil-teaching-deck 的頁型清單：封面／問題引入／迷思澄清／比較／流程／分類／案例／數據／總結／行動／過渡）
- 本頁主重點（一句話）
...
> [!important]
> 由於生圖模型的中文字渲染有上限，**每張圖建議中文字 ≤ 20 字、片段 ≤ 3 段**。超過要拆成多頁或改用 ppsh-soil-teaching-deck。
...
DRAW="python <技能資料夾>/ppsh-draw/draw.py"
SIZE="1536x1024"

# 頁 1（封面升 medium）
$DRAW "左文右圖。圖像內容：Q版機器人老師...。圖上文字：標題「把 ChatGPT 生圖偷進 Claude Code」、副標「AI 生圖 × 教學工作流」、標籤「EP15」。風格：扁平向量插畫、16:9橫版、深夜藍#0D1B2A背景、亮青藍#00C6FF主色...。避免：不要逼真照片、不要亂碼。" \
...
```bash
python -X utf8 "<技能資料夾>\ppsh-soil-image-deck\pack_pptx.py" \
...
|---------|-------------|---------|---------|
| 文字為主 + 插圖 | `ppsh-soil-teaching-deck` | pptx 文字框 | 小插圖（inset） |
| 整頁 AI 圖 | `ppsh-soil-image-deck` + `baked` | AI 生在圖裡 | 全版主視覺 |
| **AI 底圖 + 可編輯文字** | **`ppsh-soil-image-deck` + `plate`** | **pptx 文字框** | **全版設計底圖** |
